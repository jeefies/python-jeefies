...
(defclass Connect []
	(defn __init__ [self smtp-host &optional [smtp-port 465] [acc []]]
		(if* acc (setv [self.mail self.passwd] acc))
	...
	(defn send [self message]
		(.sendmail self.smtp message.sender (set message.reci) (str message))
	)
	(defn close [self]
		(.close self.smtp)
	)
